...
import { fetchTopRatedGames } from "../Services/RawgServices";
...
const TopRatedGames = () => {
  ...
  // Keywords to filter out inappropriate games
  const inappropriateKeywords = [
    ...
    "Memories",
    "explicit",
    "coffee machine", // Add any other keywords you deem inappropriate
  ];

  const isAppropriate = (game) => {
    // Check if the game's name or description contains inappropriate keywords
    const lowercaseName = game.name?.toLowerCase() || "";
    const lowercaseDescription = game.description_raw?.toLowerCase() || "";
  ...
  useEffect(() => {
    const fetchGames = async () => {
      ...
      const filteredGames = games.filter((game) => isAppropriate(game));
      setTopRatedGames(filteredGames);
      setLoading(false);
    };
    fetchGames();
  }, []);
  ...
  if (loading) {
    return <p className="text-center mt-5">Loading Top-Rated Games...</p>;
  }
  ...
  return (
    <div className="home dark-scheme">
      ...
      <div className="container mt-5">
        <h2 className="text-center mb-4 text-light">Top-Rated Games</h2>
